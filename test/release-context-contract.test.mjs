...
const CURRENT_RELEASE = "0.3.5";
const V1_REMOVAL_RELEASE = "v0.4.0";
const execFileAsync = promisify(execFile);
const CLI = resolve("bin/skillboard.mjs");

test("release identity is v0.3.5 across package metadata and changelog", async () => {
  ...
  // Then: every publish surface identifies the current v2 release as v0.3.5.
  assert.deepEqual(identities, [CURRENT_RELEASE, CURRENT_RELEASE, CURRENT_RELEASE]);
  assert.match(changelog, /^## 0\.3\.5\b/m);
});
