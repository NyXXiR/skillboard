...
test("README and install docs lead with npm quick start after registry publish", async () => {
  ...
  const install = await readFile(resolve("docs/install.md"), "utf8");
  const reference = await readFile(resolve("docs/reference.md"), "utf8");
  const readmeBeforeLegacy = readme.slice(0, readme.indexOf("## Legacy Project Policy Mode"));
  const installBeforeLegacy = install.slice(0, install.indexOf("## Deprecated Project Policy Mode"));
  const referenceBeforeLegacy = reference.slice(0, reference.indexOf("## Deprecated Project Policy Mode"));

  assert.match(readme, /## 5-Minute Quick Start/);
  assert.doesNotMatch(readmeBeforeLegacy, /\bbootstrap\b/i);
  ...
  assert.match(readme, /This setup is agent-layer only/);
  ...
  assert.doesNotMatch(readme, /npx --yes --package agent-skillboard skillboard init/);
  ...
  assert.match(install, /^# Install And Agent-Layer Setup/m);
  assert.doesNotMatch(installBeforeLegacy, /\bbootstrap\b/i);
  ...
  assert.match(install, /Deprecated Project Policy Mode/i);
  assert.match(install, /`skillboard init` is deprecated project-local policy bootstrap/i);
  assert.match(install, /skipped lifecycle scripts/);
  assert.match(install, /need to repair the\s+agent-layer guidance install/);
  assert.doesNotMatch(install, /npx --yes --package agent-skillboard skillboard init/);
  assert.doesNotMatch(install, /npx agent-skillboard init/);
  assert.match(install, /## Run Unreleased Builds From GitHub/);
  assert.match(install, /npx --yes --package github:NyXXiR\/skillboard skillboard --version/);
  assert.match(install, /npm exec --yes --package github:NyXXiR\/skillboard -- skillboard help/);
  assert.match(install, /## Install From A Clone/);
  assert.match(reference, /git clone https:\/\/github\.com\/NyXXiR\/skillboard\.git/);
  assert.match(reference, /For installation and agent-layer setup commands/);
  assert.doesNotMatch(referenceBeforeLegacy, /\bbootstrap\b/i);
  ...
  assert.match(reference, /skillboard init \[--dir <path>\] \[--scan-root <dir>\[,<dir>\]\] \[--no-scan-installed\]/);
  assert.match(reference, /Deprecated Project Policy Mode/i);
  assert.match(reference, /`skillboard init` is deprecated project-local policy bootstrap/i);
  assert.doesNotMatch(reference, /npx --yes --package agent-skillboard skillboard init/);
