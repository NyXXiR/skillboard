...
test("cli uninstall keep-settings preserves project policy and bridge guidance", async () => {
  const root = await mkdtemp(join(tmpdir(), "skillboard-uninstall-keep-settings-test-"));
  try {
    await writeFile(join(root, "AGENTS.md"), "# Existing agent rules\n\nKeep this line.\n", "utf8");
    await execFileAsync(process.execPath, ["bin/skillboard.mjs", "init", "--dir", root, "--no-scan-installed"]);
    await writeFile(join(root, ".skillboard", "reports", "user-report.md"), "keep report\n", "utf8");
    ...
    assert.match(result.stdout, /Preserved: .*`CLAUDE\.md`/);
...
test("cli uninstall keep-settings preserves generated project state directories", async () => {
  const root = await mkdtemp(join(tmpdir(), "skillboard-uninstall-keep-empty-state-test-"));
  try {
    await execFileAsync(process.execPath, ["bin/skillboard.mjs", "init", "--dir", root, "--no-scan-installed"]);
...
test("cli uninstall granular cleanup flags preserve unrequested project settings", async () => {
  const root = await mkdtemp(join(tmpdir(), "skillboard-uninstall-granular-test-"));
  try {
    await execFileAsync(process.execPath, ["bin/skillboard.mjs", "init", "--dir", root, "--no-scan-installed"]);
    await writeFile(join(root, ".skillboard", "reports", "user-report.md"), "keep report\n", "utf8");
    await writeFile(join(root, ".skillboard", "hooks", "user-hook.sh"), "#!/bin/sh\n", "utf8");
    await writeFile(join(root, "skillboard.config.yaml"), `${await readFile(join(root, "skillboard.config.yaml"), "utf8")}# user setting\n`, "utf8");

    const configResult = await execFileAsync(process.execPath, ["bin/skillboard.mjs", "uninstall", "--dir", root, "--remove-config"]);
    assert.match(configResult.stdout, /Preserved: .*`skillboard\.config\.yaml`/);
    assert.match(await readFile(join(root, "skillboard.config.yaml"), "utf8"), /# user setting/);
    assert.equal(await readFile(join(root, ".skillboard", "reports", "user-report.md"), "utf8"), "keep report\n");
    assert.equal(await readFile(join(root, ".skillboard", "hooks", "user-hook.sh"), "utf8"), "#!/bin/sh\n");

    const reportsResult = await execFileAsync(process.execPath, ["bin/skillboard.mjs", "uninstall", "--dir", root, "--remove-reports"]);
    assert.match(reportsResult.stdout, /Removed: .*`\.skillboard\/reports`/);
    assert.match(reportsResult.stdout, /Preserved: .*`skillboard\.config\.yaml`/);
    assert.match(await readFile(join(root, "skillboard.config.yaml"), "utf8"), /# user setting/);
    assert.equal(await readFile(join(root, ".skillboard", "hooks", "user-hook.sh"), "utf8"), "#!/bin/sh\n");
    await assert.rejects(readdir(join(root, ".skillboard", "reports")), /ENOENT/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("cli uninstall granular cleanup preserves a symlinked .skillboard parent without deleting its target", async () => {
  if (process.platform === "win32") {
    return;
  }

  const root = await mkdtemp(join(tmpdir(), "skillboard-uninstall-state-parent-symlink-"));
  try {
    const externalState = join(root, "external-state");
    const externalReports = join(externalState, "reports");
    await mkdir(externalReports, { recursive: true });
    await writeFile(join(externalReports, "user-report.md"), "keep report\n", "utf8");
    await symlink(externalState, join(root, ".skillboard"), "dir");

    const result = await execFileAsync(process.execPath, ["bin/skillboard.mjs", "uninstall", "--dir", root, "--remove-reports"]);

    assert.match(result.stdout, /Preserved: .*`\.skillboard\/reports`/);
    assert.equal(await readFile(join(externalReports, "user-report.md"), "utf8"), "keep report\n");
    assert.equal(await readlink(join(root, ".skillboard")), externalState);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("cli uninstall rejects mutually exclusive config removal flags", async () => {
  ...
  try {
    ...
    try {
      await execFileAsync(process.execPath, ["bin/skillboard.mjs", "uninstall", "--dir", root, "--keep-settings", "--purge"]);
    } catch (caught) {
      keepSettingsError = caught;
    }
    assert.equal(keepSettingsError?.code, 1);
    assert.match(keepSettingsError.stderr, /--keep-settings cannot be combined with --purge/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("cli uninstall default deletes modified config while keep-settings preserves it", async () => {
  ...
  try {
    ...
    const kept = await execFileAsync(process.execPath, ["bin/skillboard.mjs", "uninstall", "--dir", root, "--keep-settings"]);

    assert.match(kept.stdout, /Preserved: .*`skillboard\.config\.yaml`/);
